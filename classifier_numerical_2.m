function output = classifier_numerical_2(db, idx, output)
    % Function for fitting the numerical classifiers on the AMLC features.

    % Number of classifiers
    m = 4;
    
    % Extract the partition and the AMLC feature data
    X = db(idx).X(:, 9:end);
    Y = db(idx).Y;
    
    X_v = db(idx).X_v(:, 9:end);
    Y_v = db(idx).Y_v;
    ...
    opts = statset();
    inmodel = sequentialfs(@svm_function, X, Y, 'cv', 5, 'options', opts);
    
    X = X(:, boolean(inmodel));
    X_v = X_v(:, boolean(inmodel));
...
function [loss] = svm_function(X, Y, X_v, Y_v)
    % SVM function for calculating the AUC loss for the feature selector

    svm_model = fitcsvm(X, Y, 'KernelFunction', 'linear', 'Prior', 'uniform');
     
    [~, scores, ~] = predict(svm_model, X_v);
    
    [~, ~, ~, AUC] = perfcurve(Y_v, scores(:, 2), 2, 'Prior', 'uniform');
